...
layout (location = 0) uniform sampler2D srcTexture;
uniform vec2 srcResolution;
uniform int mipLevel;
uniform bool usePrefilter;
uniform float prefilterThreshold;
...
vec3 prefilter(vec3 c) {
    ...
    float contribution = max(0, brightness -  prefilterThreshold);
...
void main()
{
    ...
    // vshader_post uses tc instead of texCoord.
    vec2 texCoord = tc;
   ...
   // Check if we need to perform Karis average on each block of 4 samples
    ...
    case 0:
        ...
        // Filter low brightness pixels to avoid blurry image.
        if(usePrefilter)
    ...
    // Make sure downsample is not zero.
    // If so any future mix with this pixel will yield 0,
    // which will create black spots in the blurred texture.
